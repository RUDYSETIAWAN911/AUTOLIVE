...
const Dashboard = () => {
  ...
  const handleUpgradeClick = () => { toast.info(t('upgrade_to_access')); navigate('/payment') }
  ...
  const handleMenuClick = (item) => {
    ...
    if (!isAvailable) { toast.error(`${language === 'id' ? 'Fitur' : 'Feature'} ${item.titleId ? t(item.titleId) : item.titleEn} ${t('upgrade_to_access')}`); return }
    navigate(item.path)
  }
    ...
    <div className="min-h-screen bg-light dark:bg-dark transition-colors duration-300">
      <div className="bg-light-card dark:bg-dark-card border-b border-gray-200 dark:border-gray-800 p-4 sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3"><button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800"><Menu className="w-5 h-5" /></button>{logoUrl ? <img src={logoUrl} alt="Logo" className="h-8 w-auto" /> : <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-primary to-secondary flex items-center justify-center"><span className="text-white font-bold text-sm">A</span></div>}<h1 className="text-2xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent cursor-pointer" onClick={() => navigate('/dashboard')}>AUTOLIVE</h1></div>
          <div className="flex items-center gap-3"><LanguageToggle /><ThemeToggle /><div className="hidden md:flex items-center gap-2"><img src={user.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${user.email}&background=E63946&color=fff`} alt="Avatar" className="w-8 h-8 rounded-full object-cover" /><span className="text-gray-700 dark:text-gray-300 text-sm">{user.email}</span>{getSubscriptionBadge()}</div><button onClick={handleLogout} className="p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800"><LogOut className="w-5 h-5 text-red-500" /></button></div>
        </div>
      </div>
      <div className="flex">
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-light-card dark:bg-dark-card border-r border-gray-200 dark:border-gray-800 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="p-4 flex justify-between items-center lg:hidden border-b"><span className="font-bold">Menu</span><button onClick={() => setSidebarOpen(false)}><X className="w-5 h-5" /></button></div>
          <nav className="p-4 space-y-2">{menuItems.map((item, idx) => {
            const isAvailable = typeof item.isAvailable === 'function' ? item.isAvailable(userRole) : item.isAvailable
            return (<button key={idx} onClick={() => handleMenuClick(item)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 hover:scale-105 ${isAvailable ? 'hover:bg-gradient-to-r from-primary/10 to-secondary/10' : 'opacity-50 cursor-not-allowed'}`}><item.icon className="w-5 h-5 text-primary" /><span className="text-gray-700 dark:text-gray-200">{t(item.titleId)}</span>{!isAvailable && <Lock className="w-3 h-3 text-gray-400 ml-auto" />}</button>)
          })}</nav>
        </aside>
        <main className="flex-1 p-6 lg:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{menuItems.map((item, idx) => {
            const isAvailable = typeof item.isAvailable === 'function' ? item.isAvailable(userRole) : item.isAvailable
            return (<div key={idx} onClick={() => handleMenuClick(item)} className={`bg-light-card dark:bg-dark-card rounded-xl p-4 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:scale-105 cursor-pointer group ${isAvailable ? 'hover:border-primary/50' : 'opacity-60'}`}><div className="flex items-start gap-3"><div className={`inline-flex p-2 rounded-lg bg-gradient-to-r ${item.color} bg-opacity-10 group-hover:bg-opacity-20 transition`}><item.icon className="w-5 h-5 text-primary" /></div><div className="flex-1"><h4 className="text-sm font-semibold text-gray-800 dark:text-white">{t(item.titleId)}</h4><p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{language === 'id' ? 'Klik untuk mengakses' : 'Click to access'}</p></div>{!isAvailable && <Lock className="w-4 h-4 text-gray-400" />}</div>{!isAvailable && <div className="mt-2 text-xs text-primary text-center">🔒 {t('upgrade_to_access')}</div>}</div>)
          })}</div>
          <div className="mt-8 pt-4 border-t border-gray-200 dark:border-gray-800 text-center"><p className="text-xs text-gray-500">© 2026 AUTOLIVE - {t('auto_content')}</p></div>
        </main>
      </div>
    </div>
  )
