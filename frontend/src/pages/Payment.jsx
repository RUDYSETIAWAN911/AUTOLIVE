...
import { ArrowLeft, CreditCard, Building, Wallet, Copy, CheckCircle, DollarSign } from 'lucide-react'
...
const Payment = () => {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      const amount = localStorage.getItem('selected_amount')
  ...
  const price = selectedPlan === 'pro' ? 3 : 5
  const priceIdr = selectedPlan === 'pro' ? 45000 : 75000
  ...
  const confirmPayment = async () => {
    ...
    setTimeout(async () => {
      ...
      else { toast.success(`Selamat! Anda sekarang member ${selectedPlan?.toUpperCase()}`); localStorage.removeItem('selected_plan'); localStorage.removeItem('selected_amount'); navigate('/dashboard') }
      setProcessing(false)
    }, 2000)
  }
