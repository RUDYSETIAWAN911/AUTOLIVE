...
import { ArrowLeft, UserPlus, Mail, Phone, Lock, Eye, EyeOff, Key, CheckCircle, AlertCircle, Shield, Crown, Star, DollarSign } from 'lucide-react'
...
const Register = () => {
  ...
  const sendVerificationCode = async () => {
    ...
    // Cek status email
    const existingSubscription = await checkEmailStatus(formData.email)
    
    // Logika deteksi duplikat
    if (existingSubscription) {
      if (selectedPlan === 'free') {
        if (existingSubscription === 'free') {
          toast.error(t('email_exists_free'))
          return
        } else if (existingSubscription === 'pro') {
          toast.info(t('email_exists_pro'))
          // Tetap lanjutkan untuk upgrade
        } else if (existingSubscription === 'premium') {
          toast.info(t('email_exists_premium'))
          // Tetap lanjutkan untuk upgrade/downgrade
        }
      } else if (selectedPlan === 'pro') {
        if (existingSubscription === 'premium') {
          toast.info('Email terdaftar sebagai PREMIUM. Anda dapat mendaftar sebagai PRO (downgrade).')
        } else if (existingSubscription === 'free') {
          toast.info('Email terdaftar sebagai FREE. Silakan upgrade melalui dashboard.')
          return
        }
      } else if (selectedPlan === 'premium') {
        if (existingSubscription === 'pro') {
          toast.info('Email terdaftar sebagai PRO. Anda dapat mendaftar sebagai PREMIUM (upgrade).')
        } else if (existingSubscription === 'free') {
          toast.info('Email terdaftar sebagai FREE. Silakan upgrade melalui dashboard.')
          return
        }
    ...
    setLoading(true)
    try {
      // Kirim OTP via Supabase Auth (email magic link)
      ...
      // Simpan kode ke localStorage untuk verifikasi (OTP dari email)
      // User akan memasukkan kode yang diterima dari email
  ...
  const verifyCode = async () => {
    ...
    if (error) { 
      toast.error(t('verification_failed') + ': ' + error.message)
      return false
    }
    return true
  }
  ...
  const handleRegister = async () => {
    ...
    // Cek status email lagi
    const existingSubscription = await checkEmailStatus(formData.email)
    
    if (existingSubscription) {
      if (selectedPlan === 'free' && existingSubscription !== 'free') {
    ...
    try {
      ...
      if (authData.user) {
        // Hitung subscription expiry (free 7 hari)
        ...
        if (selectedPlan === 'pro' || selectedPlan === 'premium') {
          ...
          localStorage.setItem('selected_amount', prices[selectedPlan].toString())
          navigate('/payment')
        } else {
          navigate('/login')
        }
  ...
  const plans = [
    { id: 'free', name: 'Free Trial', price: '0', priceUsd: '0', duration: '7 hari', features: ['Akses semua fitur', 'Masa aktif 7 hari'], icon: <User className="w-5 h-5" />, color: 'from-gray-500 to-gray-700' },
    { id: 'pro', name: 'Pro', price: '3', priceUsd: '$3', duration: 'per bulan', features: ['Akses semua fitur', 'Kecuali jadwal upload'], icon: <Star className="w-5 h-5 text-blue-500" />, color: 'from-blue-500 to-cyan-500' },
    { id: 'premium', name: 'Premium', price: '5', priceUsd: '$5', duration: 'per bulan', features: ['Akses semua fitur lengkap', 'Prioritas dukungan'], icon: <Crown className="w-5 h-5 text-yellow-500" />, color: 'from-yellow-500 to-orange-500' }
  ]
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4"><h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">{t('select_plan')}</h2>
            {plans.map((plan) => (
              <div key={plan.id} onClick={() => setSelectedPlan(plan.id)} className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedPlan === plan.id ? `border-primary bg-gradient-to-r ${plan.color} bg-opacity-10` : 'border-gray-200 dark:border-gray-800 hover:border-primary/50'}`}>
                <div className="flex justify-between items-center"><div><div className="flex items-center gap-2"><div className="p-1 rounded-lg bg-primary/10">{plan.icon}</div><h3 className="text-lg font-bold">{plan.name}</h3></div><p className="text-2xl font-bold text-primary">{plan.priceUsd} USD</p><p className="text-xs text-gray-500">{plan.duration}</p></div>{selectedPlan === plan.id && <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center text-white">✓</div>}</div>
          ...
          <div className="bg-light-card dark:bg-dark-card rounded-xl p-6 border"><h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6">{t('register')}</h2>
            <div className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('full_name')}</label><div className="relative"><UserPlus className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" name="full_name" value={formData.full_name} onChange={handleChange} className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900" placeholder={language === 'id' ? 'Nama lengkap' : 'Full name'} required /></div></div>
